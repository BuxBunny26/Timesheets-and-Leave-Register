...
export default function StatusBadge({ status }: { status: string }) {
  return (
    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium capitalize ${colours[status] ?? 'bg-[var(--surface-secondary)] text-[var(--text-secondary)]'}`}>
      {status.replace(/_/g, ' ')}
    </span>
  )
}
